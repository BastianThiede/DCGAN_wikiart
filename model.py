...
def generator(input_dim=100,units=1024,activation='relu'):
    ...
    # FC: 2x2x512
    generator.add(Dense(2* 2* 128,input_shape=(input_dim,), kernel_initializer=init))
    generator.add(Reshape((2, 2, 128)))
    generator.add(ReLU())
    generator.add(UpSampling2D())

    # Conv 2: 8x8x128
    generator.add(Conv2DTranspose(1024, kernel_size=5, strides=2, padding='same'))
    generator.add(BatchNormalization(momentum=momentum_value))
    generator.add(ReLU())
